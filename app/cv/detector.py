...
class YOLOObjectDetector:
    ...
    def __init__(self, confidence_threshold: float = 0.5):
        ...
        self.is_enabled = os.getenv(
            "ENABLE_COMPUTER_VISION",
            "false").lower() == "true"
    ...
    def _load_model(self):
        ...
        if not self.is_enabled:
            raise RuntimeError(
                "Computer Vision is disabled in configuration (ENABLE_COMPUTER_VISION=false)")
    ...
    def detect(
            self, image_array: np.ndarray) -> Tuple[List[Dict[str, Any]], float]:
        """
        ...
        # verbose=False to prevent ultralytics from spamming stdout on every
        # request
        ...
        # Though for some cases count matters, we just need to know *what* is
        # in the image.
        unique_objects = {}
